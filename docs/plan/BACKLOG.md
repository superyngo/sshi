...
| B20 | 2026-09-25 | 2026-09-25 | P1 | Auth tries neither `ssh-agent` nor OpenSSH default keys; `IdentitiesOnly` ignored; one `IdentityFile` kept. `reference/config-schema.md` claims agent auth | `src/config/ssh_config.rs` `ParsedSshConfig::query`; `src/host/auth.rs` `authenticate` | M | **Decided 2026-09-25: OpenSSH order — agent (`SSH_AUTH_SOCK`), every `IdentityFile`, default keys only if none listed, passphrase only for encrypted keys, password unless `IdentitiesOnly`; Windows agent pipe deferred; after B57.** A host reachable by `ssh` via `~/.ssh/id_ed25519` or an agent key authenticates without a prompt; `IdentitiesOnly yes` skips password fallback |
