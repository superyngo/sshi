...
| B73 | 2026-09-25 | 2026-09-25 | P3 | sh `swap` probe is `free -b` only, so macOS/BSD hosts never report swap (checkout shows `-`), unlike the memory/disk/load probes fixed in B39 (found while verifying B39) | `src/metrics/probes/sh.rs` `command_for`; `src/metrics/parser.rs` | S | macOS `sysctl vm.swapusage` output parsed into total/used; fixture test; real binary shows swap for a macOS host |
...
|---|---|---|
| B69 | `-v/--verbose` had almost no observable CLI effect; russh `log` records never reached the subscriber | HASH-B69 — `VERBOSE_FILTER` (`sshi=debug,russh=info,info`); `try_init` installs the `log`→tracing bridge; debug events in `connect_one`, `authenticate`, `RusshSessionPool::exec`; `Context::verbose` and its constructor params removed; real binary: `run -a -v` 5 → 13 lines (connect/auth/exec), `RUST_LOG=russh=debug` 151 lines (was WARN only) |
