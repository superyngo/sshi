...
|---|---|---|
| B3 | Config tab breadcrumb indexed `config.host/check/sync[*i]` directly; stale index panicked | HASH-B3 — `.get(*i)` with `?` fallback in `ConfigTabState::breadcrumb`; test `breadcrumb_stale_index_does_not_panic` |
