...
|---|---|---|
| B61 | Enums/catalogs re-spelled: shell strings in Config tab, `ShellMode` label ×3, check catalog ×2 (×3 with the B16 template), script-extension mapping ×2 | HASH-B61 — one source each; `inject_config_comments` builds the probe list from `DEFAULT_CHECK_ENABLED`; real binary: `init` template byte-identical probe list |
