...
|---|---|---|
| B4 | `HostEntry` has no stable `id`; Config selection restore after delete was positional | HASH-B4 — `EntryKey::{Host(name), Id}` in `ConfigSelectionSnapshot`; `selected_entry_key` / `find_sidebar_idx_by_key`; test `snapshot_restores_host_by_name` (replaces the test that pinned the positional gap); real TUI: external edit removing `h1` keeps the cursor on `h3` (old: moved to `h4`) |
