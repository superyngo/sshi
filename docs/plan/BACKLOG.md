...
| B20 | 2026-09-25 | 2026-09-25 | P1 | Auth tries neither `ssh-agent` nor OpenSSH default keys; `IdentitiesOnly` ignored; one `IdentityFile` kept. `reference/config-schema.md` claims agent auth | `src/config/ssh_config.rs` `ParsedSshConfig::query`; `src/host/auth.rs` `authenticate` | M | A host reachable by `ssh` via `~/.ssh/id_ed25519` or an agent key authenticates without a prompt; `IdentitiesOnly yes` skips password fallback |
| B21 | 2026-09-25 | 2026-09-25 | P1 | Stdout previews byte-sliced: `sshi log` and View → Log panic on non-ASCII output (reproduced, exit 101) | `src/commands/log.rs` `run`; `src/tui/tabs/view_tab.rs` `render_log_result` | S | The reproduction row in the 2026-09-25 code audit prints a truncated preview; one shared char-boundary truncate used by both |
| B22 | 2026-09-25 | 2026-09-25 | P1 | `checkout --combined-view` applies `LIMIT 50` globally, dropping hosts (reproduced) | `src/commands/checkout/core.rs` `fetch_combined_snapshots` | S | 60 + 1 snapshot reproduction shows `server2` online with its value; limit is per host |
| B23 | 2026-09-25 | 2026-09-25 | P1 | Config tab `strip_unit` trims trailing `s`/`d`/`%` from every text field on edit-open, corrupting names and paths | `src/tui/tabs/config_tab.rs` `strip_unit`, `activate_inline_edit`, `handle_entry_form_key` | S | Open-edit-Enter on host `prod` keeps `prod`; unit stripping only for unit-bearing numeric fields |
...
| B25 | 2026-09-25 | 2026-09-25 | P1 | Sync `conflict_strategy = skip` conflicts are counted as in sync | `src/commands/sync/decide.rs` `make_decisions`; `src/commands/sync/mod.rs` (empty-decisions branch) | S | A conflicting file under `skip` appears as a skipped conflict in the summary, not in `files_synced` |
...
| B35 | 2026-09-25 | 2026-09-25 | P2 | Exit status is 0 when every host fails (documented in `reference/cli.md`); scripts cannot detect fleet failure | `src/main.rs`; command wrappers in `src/commands/` | S | Decision recorded (ADR); chosen exit code implemented and documented |
...
| B44 | 2026-09-25 | 2026-09-25 | P2 | `XDG_CONFIG_HOME`/`XDG_STATE_HOME` ignored; macOS paths hard-coded (`wens-dev-principles cli 2`, MUST) | `src/config/app.rs` `config_dir`; `src/state/db.rs` `state_dir` | M | XDG variables honoured with legacy fallback, or ADR recording the deviation |
...
| B57 | 2026-09-25 | 2026-09-25 | P2 | `cargo audit`: russh 0.44 (RUSTSEC-2026-0153/0154), rsa Marvin, unsound anyhow/lru, unmaintained paste/number_prefix, yanked spin | `Cargo.toml`, `Cargo.lock` | L | `cargo audit` clean or each remaining advisory recorded as accepted |
