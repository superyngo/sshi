...
|---|---|---|---|---|---|---|---|
| B36 | Unknown `-n` exited 0 with a wrong hint; missing explicit `-c` became an empty config | HASH-B36 — `ensure_check_names` / `ensure_sync_names` + `load_config` (init and TUI exempt); real binary: typo/missing path exit 1, `init` still creates |
