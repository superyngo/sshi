...
|---|---|---|
| B54 | Parallel implementations: TUI export vs CLI report builders (checkout), `resolve_target_names` vs `Context::resolve_hosts`, `Summary`/`SyncSummary` printing, Operate/View target rows, `parse_ssh_config`/`load_ssh_config` | HASH-B54 — each pair reduced to one function (see changelog); tests `checkout_operation_report_rows_and_summary`, `detail_lines_dedupe_and_cluster`; real binary: `checkout -a --out` and the TUI View export of the same rows produce identical JSON apart from `executed_at` |
