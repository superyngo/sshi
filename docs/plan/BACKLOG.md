...
|---|---|---|
| B37 | Per-host outcome policy diverged: log-write failure aborted `exec`/`run` but warned in `cp`/`check`; `Partial` meant skip/success/failure by command; `cp` log rows omitted errors; `check` unreachable writes outside its transaction | HASH-B37 — `report::{record_operation_log, record_operation_log_tx, host_status_to_printer_kind, host_status_to_log_status, update_summary, default_printer_sink}` used by all four; test `mapping_and_summary_follows_adr_0004`; `state-schema.md` status column updated |
