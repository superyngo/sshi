...
|---|---|---|
| B31 | Windows `--sudo` never observed the elevated command's exit status; `run --sudo --dry-run` previewed the sh form | HASH-B31 — `shell::sudo_wrap` returns `Result` (refuses PowerShell/cmd); `run_core` records a per-host error, `exec_on_host_pooled` refuses before upload; dry-runs preview per host; real binary with h1 as `powershell`: dry-run `✗ --sudo is not supported…` / `✓ would execute: sudo id -u`, run exit 3 with the refusal (old: sent `Start-Process` to the host) |
