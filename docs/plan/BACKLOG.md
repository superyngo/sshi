...
|---|---|---|
| B51 | `maybe_reload_checkout` cleared `db_stale` when the snapshot fetch failed | HASH-B51 — `db_stale = false` only on `Ok`; test `checkout_reload_keeps_db_stale_on_fetch_error` |
