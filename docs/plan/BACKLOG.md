...
|---|---|---|
| B58 | `migrate` rewrote `user_version` downward under an older binary | HASH-B58 — `migrate` bails when `user_version > CURRENT_VERSION`; `open` adds the DB path as context; test `migrate_refuses_newer_schema_and_keeps_version`; real binary: v99 DB → exit 1, version stays 99 (old binary: rewrote it to 2) |
