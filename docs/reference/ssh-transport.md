...
- **Temp file + rename**: data is written to a sibling temp file `.<name>.sshi-tmp.<pid>-<n>` (`temp_sibling`; `<n>` is a per-process upload counter, so concurrent uploads of one path — e.g. to several hosts sharing an NFS home — never share a temp file) and renamed over the destination only after every byte arrived and the file closed cleanly. An interrupted, failed or timed-out transfer leaves the existing destination untouched and removes the temp file (locally also on cancellation, via `TempGuard`).
...
- **Abandoned temp files**: a process killed with SIGKILL can leave `.<name>.sshi-tmp.<pid>-<n>` (or the pre-B77 `.<name>.sshi-tmp.<pid>`) behind. The first upload into a directory in a run lists it and removes such files whose pid is not the current process and whose mtime is at least an hour old (`sweep_stale_temps`, `STALE_TEMP_SECS`), so a concurrent run's live temp file is never touched.
