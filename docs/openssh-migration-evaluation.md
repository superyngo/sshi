...
**注意**：`raw_command()` 不做 escaping（sshi 的指令是已組好的 shell 字串），`command()` 會 escape。sshi 的所有遠端指令都是完整 shell 字串（如 `for f in ...; do ... done`），因此**必須用 `raw_command()`**。
...
- **建議 `native-mux`**：效能更優且錯誤處理更好；sshi 的使用場景（批量操作）會受益於減少 process spawn

---

## 六、openssh 與 sshi 特有問題的對應
...
sshi 現有架構支援 Windows 客戶端（`ConnectionMode::Direct`），openssh 不支援。

**影響評估**：
- sshi 的 **Windows 遠端主機**（PowerShell/Cmd shell）→ **不受影響**（這是遠端，不是客戶端）
- sshi 的 **Windows 客戶端**（從 Windows 執行 sshi）→ **受影響**

**緩解方案**：
1. 保留 `ConnectionMode::Direct` 作為 Windows fallback（feature gate）
2. 或：接受 sshi 不在 Windows 上跑（根據你的 homelab 場景判斷）

### 6.2 MaxSessions 限制

openssh 文件提到：**"the maximum number of multiplexed remote commands is 10 by default"**

sshi 的 `max_per_host_concurrency` 預設 4，一般不會超過 10。但 sync 的 Stage 3 在密集上傳時，如果一台 host 同時有多個 channel（exec + SFTP），可能逼近限制。

**緩解**：sshi 已有 `ConcurrencyLimiter`，確保 per-host 並行度不超過設定值。只要 `max_per_host_concurrency ≤ 8`（留 2 給 SFTP 和監控），就不會有問題。
...
sshi 現有代碼已經面對這個問題（ssh binary 本就有此行為），但目前透過 ControlMaster 避免了大部分連線錯誤。遷移到 openssh 的 `native-mux` 模式後，錯誤處理更精確（直接從 mux protocol 取得錯誤），所以**實際上會改善**。

### 6.4 Shell escaping

openssh 預設做 shell escaping（`command()` 方法），但 sshi 的遠端指令已經是完整的 shell 字串。

**⚠️ 必須使用 `raw_command()` 而非 `command()`**——否則 sshi 建構的 `for f in ...; do ... done` 等指令會被錯誤 escape。
...
**對 sshi 的影響**：sync 的檔案通常是 config files（小檔案），但如果有大檔案同步需求，需要用 `sftp.open()` + streaming read/write。
...
| # | 風險 | 嚴重度 | 機率 | 說明 |
|---|------|-------|------|------|
| R1 | Windows 客戶端不支援 | 🔴 | 確定 | openssh 明確不支援 Windows。如 sshi 需要在 Windows 上跑，此為 blocker |
...
- [ ] `raw_command()` vs `command()`——sshi 的所有指令必須用 `raw_command()`
...
**遷移的技術可行性：✅ 非常高**——openssh crate 的 API 與 sshi 現有架構高度匹配。
