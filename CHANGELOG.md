...
### Changed
- **chore(tui):** deleted the dead `Focusable` trait + `ArrowResult` enum + 7 adapter tests from `src/tui/focus.rs` (audit §1 P4 LOW, Phase F3 decision option (a)). The trait had zero non-test callers — production arrow dispatch is implemented per-tab. The other types in `focus.rs` (`Direction`, `Axis`, `AxisFreedom`, `FocusZone`, `EscapeOutcome`, `escape_to_parent`, `FocusPath`) are kept; they describe the focus model a future dispatch could be rebuilt on, and remain silenced by the module-level `#![allow(dead_code)]`. `rg "Focusable" src/` now returns only the historical mention in the module doc-comment. Test count: 307 (down 7 from F2's 314 — exactly the deleted adapter tests)
