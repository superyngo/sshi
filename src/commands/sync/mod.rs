...
pub async fn sync_core(
    ...
    dry_run: bool,
    cli_source: Option<&str>,
    progress: Option<&(dyn ProgressSink + Sync)>,
...
/// Phase 2: batch-collect metadata for non-recursive files and produce a
/// decision per file. Files that are already in sync (empty decision list)
/// are tallied into `summary` via `file_in_sync`; the rest are returned for
/// the distribute phase to act on.
///
/// Extracted verbatim from the legacy `sync_inner` body. Behaviour
/// byte-identical.
#[allow(clippy::too_many_arguments)]
async fn decide_batch(
    ...
    for path in all_paths {
        ...
        if decisions.is_empty() {
            let hosts_list: Vec<&str> = scoped_found.iter().map(|f| f.host.as_str()).collect();
            if verbose {
...
/// Phase 3: distribute each decision via SFTP. In dry-run mode, just emits
/// the planned transfer lines + `printer::print_host_line` calls. In real
/// mode, runs `distribute_pooled`, accumulates per-host outcomes into
/// `summary` / `host_file_map`, and writes the DB rows inside one
/// transaction (Phase B2 optimisation, preserved verbatim).
///
/// Extracted from the legacy `sync_inner` body. Behaviour byte-identical.
#[allow(clippy::too_many_arguments)]
async fn distribute_batch(
    ...
    if dry_run {
        for d in all_decisions {
            let mut all_targets: Vec<&str> = d.synced_hosts.iter().map(|s| s.as_str()).collect();
    ...
    for decision in all_decisions {
        ...
        match distribute_pooled(reachable_hosts, decision, ctx.timeout, limiter, sessions).await {
...
/// Phase 4: iterate `recursive_entries` (the `recursive = true` [[sync]]
/// entries), expand each entry's directories, and run the per-file
/// sync flow via `sync_path_across`.
///
/// Extracted verbatim from the legacy `sync_inner` body.
#[allow(clippy::too_many_arguments)]
    ...
    for (entry, hosts_for_entry, effective_source) in recursive_entries {
        ...
        let expanded_paths: Vec<String> = if let Some(src_name) = effective_source {
            if let Some(source_host) = scoped_hosts.iter().find(|h| h.name == *src_name) {
                match expand_directory_paths(source_host, &entry.paths, true, ctx.timeout, sessions)
                    .await
                {
                    Ok(expansions) => {
